...
```{r, sie-rect-plot, echo = FALSE, fig.height=5, fig.width=6}
p3 <- sea_ice_extent |> 
  filter(between(year, 1979, 2021)) |> 
  group_by(year, month2) |> 
  summarise(mean_extent = mean(extent, na.rm = TRUE)) |> 
  ungroup() |> 
  ggplot(aes(x = year, y = month2, fill = mean_extent)) +
  geom_raster() +
  ...
  paletteer::scale_fill_paletteer_c(
    ...
    guide = guide_colorbar(
      title = parse(text = "Sea~ice~extent ~ (km^2 %*% 10^6)"),
      title.position = "top",
      title.hjust = 0.5,
      barwidth = unit(10, "cm"),
...
ggplot(df, aes(x = year, y = sunspot)) +
  ...
  scale_color_manual(
    values = c("Rising" = "#E15554", "Faling" = "#4D9DE0")
  ) +
  coord_fixed(ratio = ratio) +
  theme(text = element_text(size = 6)) +
  theme(
...
df <- sea_ice_extent |>
  mutate(across(extent, .fns = list(minimum = min, maximum = max))) |> 
  group_by(year) |> 
  filter(any(extent == extent_minimum | extent == extent_maximum)) |> 
  ungroup()

df_annotation <- df |> 
  pivot_longer(contains("extent_")) |> 
  filter(extent == value) |> 
  mutate(date = as.Date(paste(2000, month, day, sep = "-"))) |> 
  mutate(name = str_remove(name, "extent_")) |> 
  mutate(label = glue::glue("All time {name}")) |> 
  mutate(description = glue::glue(str_wrap("All time {name} sea ice extent was reached in {month2} of {year}.")))

p1 <- df |>
  mutate(date = as.Date(paste(2000, month, day, sep = "-"))) |>
  ggplot(aes(x = date, y = extent, color = factor(year))) +
  geom_line(size = 1) +
  ggforce::geom_mark_circle(
    ...
    label.family = c("Montserrat", "Exo")
  ) +
  scale_x_date(date_labels = "%B") +
  scale_color_manual(values = c("#FFCB27", "#E3395A")) +
  labs(
    x = NULL,
    y = quote("Sea ice extent" ~ (km^2 %*% 10^6)),
    color = NULL,
    title = "Arctic sea ice extent",
    subtitle = "<span style='color:#FFCB27'>1979</span> and <span style='color:#E3395A'>2012</span> were the most contrasted years in terms of sea ice extent in the Arctic"
  ) +
  theme(
    panel.grid = element_line(color = "#525966"),
    panel.background = element_rect(fill = "#394457"),
    panel.border = element_blank(),
    plot.background = element_rect(fill = "#394457", color = NA),
    plot.title = element_text(color = "#d0d3d8"),
    plot.subtitle = element_markdown(
      color = "#d0d3d8",
      family = "Baloo 2",
      size = 14
    ),
    axis.title = element_text(color = "#d0d3d8"),
    axis.text = element_text(color = "#d0d3d8"),
    legend.key = element_rect(fill = "#394457"),
    legend.background = element_rect(fill = "#394457"),
    # legend.position = "bottom",
    # legend.text = element_text(color = "#d0d3d8"),
    legend.position = "none"
  )
...
sie_high <-
  st_read(
    here::here(
      ...
      "raw",
      "seaice_extent",
      "extent_N_197903_polygon_v3.0",
      "extent_N_197903_polygon_v3.0.shp"
    ),
    quiet = TRUE
  )
...
sie_low <-
  st_read(
    here::here(
      ...
      "raw",
      "seaice_extent",
      "extent_N_201209_polygon_v3.0",
      "extent_N_201209_polygon_v3.0.shp"
    ),
    quiet = TRUE
  )

p2 <- ggplot() +
  geom_sf(data = sie_high, fill = "#FFCB27", size = 0.1) +
  geom_sf(data = sie_low, fill = "#E3395A", size = 0.1) +
  coord_sf() +
  theme_minimal() +
  theme(
...
p1 +
  inset_element(p2, 0.42, 0.1, 0.3, 0.4, align_to = "panel", clip = TRUE, ignore_tag = T) &
  theme(
    plot.background = element_rect(fill = "#394457", color = NA),
...
Some color resources to procrastinate on!
...
`r chunk_reveal(chunk_name = "theme1", title = "# ggplot2 built-in themes")`
