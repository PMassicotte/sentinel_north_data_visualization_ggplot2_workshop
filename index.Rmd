...
years <-
  seq(min(sea_ice_extent$year) + 1, max(sea_ice_extent$year) - 1, len = 4) |>
  floor()

df_annotation <- sea_ice_extent |> 
  filter(year == 1979) |> 
  filter(extent == max(extent)) |> 
  mutate(date = as.Date(paste(2000, month, day, sep = "-"))) |> 
  mutate(label = glue::glue("All time maximum")) |> 
  mutate(description = glue::glue(str_wrap("All time maximum sea ice extent was reached in {month2} of {year}.")))
...
sea_ice_extent |>
  ...
  scale_x_date(date_labels = "%B") +
  scale_color_manual(
    values = c("#FFCB27", "#9e82a1", "#E3395A", "#43AFB0")
  ) +
  labs(
    x = NULL,
    y = quote("Sea ice extent" ~ (km^2 %*% 10^6)),
  ...
  theme(
    panel.grid = element_line(color = "#8b9da5"),
    panel.background = element_rect(fill = "#394457"),
    panel.border = element_blank(),
    plot.background = element_rect(fill = "#394457", color = NA),
    plot.title = element_text(color = "#8b9da5"),
    axis.title = element_text(color = "#8b9da5"),
    axis.text = element_text(color = "#8b9da5"),
